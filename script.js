document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateCountdown() {
    ...
    if (distance < 0) {
      ...
      const hoursPassed = Math.floor((now - endDate) % (1000 * 60 * 60 * 24) / (1000 * 60 * 60));
      const minutesPassed = Math.floor((now - endDate) % (1000 * 60 * 60) / (1000 * 60));
      const secondsPassed = Math.floor((now - endDate) % (1000 * 60) / 1000);

      // Set the text content for elapsed time
      document.getElementById('days').textContent = daysPassed.toString().padStart(2, '0');
      document.getElementById('hours').textContent = hoursPassed.toString().padStart(2, '0');
      document.getElementById('minutes').textContent = minutesPassed.toString().padStart(2, '0');
      document.getElementById('seconds').textContent = secondsPassed.toString().padStart(2, '0');
